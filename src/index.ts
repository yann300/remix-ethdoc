...
import { remixApi, CompilationFileSources, CompilationResult, Status } from '@remixproject/plugin-iframe'
...
export class EthdocComponent extends LitElement {
  ...
  async writeDoc(name: string) {
    try {
      ...
      await this.client.fileManager.setFile(`doc/${name}.doc.md`, content)
      this.showAlert(name)
    } catch (err) {
